...
const AllSeller = () => {
    const { data: sellers = [], isLoading, refetch } = useQuery({
        queryKey: ['sellers'],
        queryFn: async () => {
            const res = await fetch('https://backend-orpin-pi.vercel.app/sellers',{
                headers:{
    ...
    const deleteSeller = sellerData => {
        ...
        if (agree) {
            fetch(`https://backend-orpin-pi.vercel.app/allSeller/${sellerData._id}`, {
                method: "DELETE"
    ...
    const verifySeller=users=>{
        ...
        fetch(`https://backend-orpin-pi.vercel.app/allSeller/${users._id}`, {
            method: "PUT"
        })
            .then(res => res.json())
            .then(data => {
