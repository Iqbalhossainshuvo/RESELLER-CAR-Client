...
const MyProducts = () => {
    ...
    const { data: myProduct = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://backend-orpin-pi.vercel.app/myProducts?email=${user?.email}`,{
                headers:{
    ...
    const deleteButton = products => {
        ...
        if (agree) {
            fetch(`https://backend-orpin-pi.vercel.app//deleteMyProduct/${products._id}`, {
                method: "DELETE"
            })
                .then(res => res.json())
                .then(data => {
